...
describe('IPC Handlers', () => {
  ...
  describe('Settings Handlers', () => {
    ...
    it('SETTINGS_SAVE_ONE saves setting', async () => {
      ...
      const result = await handlers[channels.SETTINGS_SAVE_ONE](null, {
        key: 'theme',
        value: 'light',
      });

      expect(db.execute).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO settings'), [
        'theme',
        'light',
        'light',
      ]);
  ...
  describe('Item Handlers', () => {
    ...
    it('ITEM_UPDATE checks for duplicate name', async () => {
      queries.items.checkName.mockReturnValue(true);
      const result = await handlers[channels.ITEM_UPDATE](null, {
        id: 1,
        data: { name: 'Duplicate' },
      });
      expect(result).toEqual({ success: false, error: 'An item with this name already exists' });
  ...
  describe('Customer Handlers', () => {
    ...
    it('CUSTOMER_UPDATE checks for duplicate NIC', async () => {
      queries.customers.checkNic.mockReturnValue(true);
      const result = await handlers[channels.CUSTOMER_UPDATE](null, {
        id: 1,
        data: { nic: '12345' },
      });
      expect(result).toEqual({ success: false, error: 'A customer with this NIC already exists' });
  ...
  describe('Sale Handlers', () => {
    ...
    it('SALE_CREATE creates sale successfully', async () => {
      ...
      const result = await handlers[channels.SALE_CREATE](null, {
        customer_id: 1,
        items: [{ id: 1 }],
      });
  ...
  describe('Purchase Handlers', () => {
    ...
    it('PURCHASE_CREATE creates purchase successfully', async () => {
      ...
      const result = await handlers[channels.PURCHASE_CREATE](null, {
        supplier_id: 1,
        items: [{ id: 1 }],
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('SUPPLIER_UPDATE returns error if id is invalid', async () => {
      ...
      queries.suppliers.update.mockImplementation(() => {
        throw new Error('DB Error');
      });
      const result = await handlers[channels.SUPPLIER_UPDATE](null, {
        id: 999,
        data: { name: 'New Name' },
      });
      expect(result).toEqual({ success: false, error: 'DB Error' });
  ...
  describe('Additional Edge Cases', () => {
    it('SUPPLIER_CREATE fails without required fields', async () => {
      ...
      queries.suppliers.create.mockImplementation(() => {
        throw new Error('NOT NULL constraint failed: suppliers.name');
      });
    ...
    it('CUSTOMER_CREATE fails without required fields', async () => {
      queries.customers.create.mockImplementation(() => {
        throw new Error('NOT NULL constraint failed: customers.name');
      });
  ...
  describe('Supplier Bill Handlers', () => {
    it('SUPPLIER_BILL_CREATE creates bill', async () => {
      ...
      const result = await handlers[channels.SUPPLIER_BILL_CREATE](null, {
        supplierId: 1,
        amount: 1000,
      });
  ...
  describe('Report Handlers', () => {
    it('REPORT_LEDGER calls correct query', async () => {
      ...
      expect(queries.reports.getLedger).toHaveBeenCalledWith(
        1,
        undefined,
        '2023-01-01',
        '2023-01-31'
